...
public class MedicineButton : MonoBehaviour
{
    ...
    public int startGoldByUpgrade = 600;

    [HideInInspector]
    // 지금 현재의 아이템 가격
    public int currentCost = 600;

    public int startCurrentCost = 600;
    ...
    void Start() { 
        ...
        */
        // 시작할 때 저장된 데이터를 가져옴
        DataController.GetInstance().LoadMedicineButton(this);
        UpdateUI();
    }

    public void PurchaseUpgrade() {
        // 싱글톤이기에 해당 변수를 선언하지 않고 클래스 이름에 dot을 찍어 접근할 수 있음 
        if(DataController.GetInstance().GetGold() >= 600) // gold가 충분하면
        {
            //== 건강 신호등이 red (2) 일 때 - 일단 주석 처리 ==//
            if (DataController.GetInstance().GetLight() == 2) {
                DataController.GetInstance().SubGold(600);
                DataController.GetInstance().SetLight(0);
                // == 포인트를 받아오는 것 처럼 lv을 받아와야 한다 
                // int lv = (int)DataController.GetInstance.GetLv();

                // level = 50*(2*lv + 1); // 약 한번 먹으면 경험치 절반 채워줌 
                ...
                DataController.GetInstance().SetLvMedicine();
                UpdateUI();
                // 아이템을 구매했을 때 저장함
                DataController.GetInstance().SaveMedicineButton(this);
            }
        }
    }
